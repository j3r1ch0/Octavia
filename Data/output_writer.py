...
def write_to_file():
	file_path = raw_input(YELLO + "Output file path: " + ENDC)
	print
	print YELLO + "[INFO] Writing to the output file..." + ENDC
	f = open(file_path,"w")
	f.write("\n".join(output_list))#Write the list to an external file
...
def ask():
	...
	print BOLD + GREEN + "[2] " + YELLO  + "Write the output list in a file" + ENDC
	print BOLD + GREEN + "[3] " + YELLO  + "Quit" + ENDC
	print	
	print HEADER + "***********************************************************" + ENDC	
	valid = False
	while (valid == False):	
		choice = raw_input(BOLD + GREEN + "0ctavia-$: " + ENDC)
		if (choice == '1'):
			dump_list()
			valid = True
		elif (choice == '2'):
